...
import { listPersonas, STRINGS } from '@content';
import { generatePersona, personaGenInputSchema } from './ai/persona-gen';
import { makeLlm } from './ai/llm';
import { logger } from './log';
import { genCode } from './game/state';
import type { Env } from './env';
import type { HealthRes } from '@shared';
export { RoomDO } from './room-do';
export { QuotaDO, EgressDO } from '@narre/cf';
...
// 쿼터는 인스턴스 하나(global)를 공유한다 — IP 한도와 LLM 프로바이더 일일 한도가 같은 DO에 산다.
function quotaStub(env: Env) {
  return env.QUOTA_DO.get(env.QUOTA_DO.idFromName('global')) as unknown as {
...
app.post('/api/personas/generate', async (c) => {
  ...
  if (ip) {
    const quota = quotaStub(c.env);
    const { ok } = await quota.incr(`persona-gen:${ip}`, PERSONA_GEN_LIMIT, PERSONA_GEN_TTL_MS);
    if (!ok) return c.json({ error: STRINGS.errors.personaGenQuota }, 429);
  }
  ...
  try {
    // 방 밖 호출이라 roomCode가 없다. 쿼터는 같은 global QuotaDO를 쓴다.
    const persona = await generatePersona(makeLlm(c.env, quotaStub(c.env), undefined), parsed.data);
...
app.post('/api/share', async (c) => {
  ...
  if (ip) {
    const quota = quotaStub(c.env);
    const { ok } = await quota.incr(`share-create:${ip}`, SHARE_CREATE_LIMIT, SHARE_CREATE_TTL_MS);
    if (!ok) return c.json({ error: STRINGS.errors.rateLimited }, 429);
  }
...
app.post('/api/feedback', async (c) => {
  ...
  if (ip) {
    const quota = quotaStub(c.env);
    const { ok } = await quota.incr(`feedback:${ip}`, FEEDBACK_LIMIT, FEEDBACK_TTL_MS);
    if (!ok) return c.json({ error: STRINGS.errors.rateLimited }, 429);
  }
...
app.post('/api/rooms', async (c) => {
  ...
  const quota = quotaStub(c.env);
  const { ok } = await quota.incr(`room-create:${ip}`, ROOM_CREATE_LIMIT, ROOM_CREATE_TTL_MS);
  if (!ok) return c.json({ error: STRINGS.errors.rateLimited }, 429);
