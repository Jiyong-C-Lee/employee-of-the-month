// 3종 AI 호출 오케스트레이션: 체인 시도 → mock 폴백 + 판정 검증.
// 원본 server/sycophant/ai.js 이식 — withFallback·advisorTurnsBatch(재시도·approach 재배정)·
// judgeSpeeches(익명 마스킹·복원)·makeEpilogue 로직 동일.
//
// LLM 호출은 게임이 주입한 deps.llm(@narre/llm 체인을 감싼 클로저)이 맡는다. env·quotaTake·
// roomCode는 그 클로저가 이미 들고 있어서 여기 시그니처에서 사라졌다(worker/ai/llm.ts).
import type { Llm } from '@narre/cf';
import { ADVISOR_SPEECH_MAX_CHARS, type Difficulty, type Situation, type Verdict } from '@shared';
...
export interface Deps {
  llm: Llm;
  /** 실 키 유무. false면 체인을 건너뛰고 바로 게임 고유 mock을 쓴다(원본 hasAnyKey 자리). */
  hasKey: boolean;
...
async function withFallback<T>(
  ...
  if (deps.hasKey) {
    try {
      ...
      // 왜 떨어졌는지 남긴다. 원본은 조용히 삼켰는데, 그러면 프로덕션에서 대사가 mock으로
      // 나와도 원인을 못 본다(이관 스모크에서 실제로 겪었다).
      logger.chainFallback({ error: e instanceof Error ? e.message : String(e), roomCode: deps.roomCode });
    ...
    async () => {
      ...
      for (let attempt = 0; attempt < 2; attempt++) {
        const { raw, provider } = await deps.llm(
          ...
          { kind: 'advisors', validate: (r) => { advisorBatchOut.parse(r); } },
        );
